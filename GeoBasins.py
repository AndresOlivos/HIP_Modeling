...
# Description: Delineates basins draining directly into the ocean and sub-basins based on user-defined parameters
# Inputs: geodatabase workspace, flow direction (D8), flow accumulation, stream order raster (Shreve)
# Parameters: minimum basin area (in sqkm), segmentation order (by Shreve), and parallel processing factors
# Outputs: basin polygons, sub-basins polygons
# ---------------------------------------------------------------------------------------------------------------------#
...
basins = GetParameterAsText(6) # Output basins
sub_basins = GetParameterAsText(7) # Output sub-basins
...
## Delineate basins draining directly in the ocean
basins_ras = Basin(fdir)

# Vectorize basins
conversion.RasterToPolygon(basins_ras, basins, "NO_SIMPLIFY", "Value", "SINGLE_OUTER_PART", None)
